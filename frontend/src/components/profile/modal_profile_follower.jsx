    ...
    try {
     ...
     const response = await requestAxios.get(`/api/auth/check`)
        ...
        for(let i = 0; i < follower.length; i++){
          ...
          await requestAxios.patch(`api/auth/getF4Fprofile`, {username:follower[i]})
            .then(response => {
          ...
          <div className="loading_spinner" style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "30vh" }}>
            ...
            followerPeople.map((a,i) => {
              return(
                ...
                <img className="main_profileImage main_profileImage_heart" alt="" src={Profile[i]} /></div><div className="main_username main_username_heart" style={{color: "#515151"}}>{a}</div>
              </div>
              );
              })
            )}
